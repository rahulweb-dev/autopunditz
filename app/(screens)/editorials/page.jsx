...
export default async function EditorialsPage() {
  ...
  try {
    ...
    const raw = await Blog.find({
      status: "published",
      category: { $regex: /^editorials$/i },
      subCategory: { $regex: /^editorials$/i },
    })
      .sort({ createdAt: -1 })
      .limit(50)
      .select("title slug content createdAt ogImage author")
      .lean();

    editorials = raw.map((b) => ({
      _id: b._id.toString(),
      title: b.title,
      slug: b.slug,
      content: b.content,
          ...
          <div className="max-w-7xl mx-auto px-4 w-full">
            <div className="max-w-3xl">
              ...
              <p className="text-white/80 text-base md:text-lg leading-relaxed">
                Explore expert perspectives, deep industry analysis, automobile
                market insights, and editorial opinions from our automotive
                newsroom.
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-14">
        ...
        {!editorials.length && (
          <div className="bg-white rounded-3xl border shadow-sm py-24 text-center">
            ...
            <h3 className="text-3xl font-bold mb-3">No Editorials Found</h3>
            <p className="text-gray-500">
        ...
        {!!editorials.length && (
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-8">
            {editorials.map((item, index) => (
              <Link key={item.slug} href={`/editorials/${item.slug}`} className="group">
